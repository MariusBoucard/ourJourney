  ...
  <div>
    ...
    <div class="songContainer">
      <div class="playerCard">
        ...
        <img :src="getPathCover(song.pathcover)" alt="cover" class="cover playerCard-img">
        <div class="buttons">
          ...
          <button @click="playSong()">
            <svg fill="#000000" height="30px" width="30px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
              ...
              <g>
                <g>
                  <path
                    d="M176.416,129.792v252.416L395.024,256L176.416,129.792z M192.416,157.504L363.024,256l-170.608,98.496V157.504z" />
                </g>
              </g>
            </svg>
          </button>
      ...
      <div class="playerCard">
        ...
        <div v-for="element, index in albumSongs" :key="element.id" class="song-row">
    <div @click="navigateToAlbum(element.songbacktitle)">
      <!-- Use a div or similar element for the clickable area that navigates -->
      <!-- <img :src="getCoverURL(element)" alt="Cover" style="width: 50px; height: 50px;"> -->
      <p>
        {{ element.titre }}
      </p>
    </div>
    <img class="trash" src="/assets/player/play.svg" @click.stop="playASongFromAlbum(index)" alt="Jouer maintenant">
  </div>
<div>
  <div class="buttons">

<button title="Ajouter l'album à la Playlist" @click="addAlbumToPlaylist()">
  <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 50 50">
    <path
      d="M 25 2 C 12.264481 2 2 12.264481 2 25 C 2 37.735519 12.264481 48 25 48 C 37.735519 48 48 37.735519 48 25 C 48 12.264481 37.735519 2 25 2 z M 25 4 C 36.664481 4 46 13.335519 46 25 C 46 36.664481 36.664481 46 25 46 C 13.335519 46 4 36.664481 4 25 C 4 13.335519 13.335519 4 25 4 z M 24 13 L 24 24 L 13 24 L 13 26 L 24 26 L 24 37 L 26 37 L 26 26 L 37 26 L 37 24 L 26 24 L 26 13 L 24 13 z">
    </path>
...
<button title="Jouer l'album" @click="playAlbum()">
  <svg fill="#000000" height="30px" width="30px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
    xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve">
    <g>
      <g>
...
export default {
  ...
  watch: {
    '$route': {
      immediate: true,
      handler() {
        // Perform actions here, like fetching new data based on the route parameter
        this.fetchData();
      }
    }},
  data() {
  ...
  methods: {
    navigateToAlbum(songbacktitle) {
      this.$router.push(`/album/${songbacktitle}`);
    },
    playAlbum(){
  // Step 1: Shift the first song from the albumSongs array
  
  // Step 2: Play the first song directly
  // Assuming playASongFromAlbum takes an index or song object. Adjust accordingly.
  this.playASongFromAlbum(0);

  // Step 3: Iterate over the remaining songs in albumSongs
  // If you need to do something with the rest of the songs, like adding them to a queue
  this.albumSongs.slice(0).forEach(song => {
    this.$store.commit('addSongToPlaylist', song);
  });
    },
    playASongFromAlbum( index){
      console.log(index);
      this.$store.commit('setCurrentSongLink', this.baseURL + "/song/" + this.albumSongs[index].songbacktitle + ".wav");
      this.$store.commit('setCurrentSong', this.albumSongs[index]);
    },
    addAlbumToPlaylist(){
    ...
    AxiosInstance.get(`/song/${id}`)
      .then((response) => {
        ...
        console.log(this.song);
         // Fetch all the song from the same album
        AxiosInstance.get('/songsAlbum/' + this.song.titrealbum)
          .then((response) => {
            this.albumSongs = response.data;
           this.albumSongs = this.albumSongs.filter(song => song.numeroalbum !== 0)
           this.albumSongs = this.albumSongs.sort((a, b) => a.numeroalbum - b.numeroalbum)
          })
          .catch((error) => {
            console.error(error);
          });
...
.songContainer {
  ...
  display: flex;
  flex-direction: row;
  /* This will align the children (the image and the song info) in a row */
  align-items: center;
  /* This will vertically center the children */
  gap: 20px;
  /* Adjusts the space between the image and the song info */
}
...
.playerCard {
  ...
  gap: 10px;
  z-index: 1;
  /* Ensures the card is above the lyrics div */
}

.cover {
  width: 80%;
  /* Adjusts the width of the cover image. You can set this to fit your design */
  height: auto;
...
.songInfo {
  flex: 1;
  /* Allows the song info section to take up the remaining space */
}
...
.playerCard {
  ...
  /* Space out the text, image, and buttons */
  margin: auto;
  /* Center the card */
...
.playerCard-img {
  ...
  /* Default border */
  transition: width 0.5s ease, border 0.5s ease, transform 0.5s ease;
...
.lyricsDiv {
  background: linear-gradient(rgba(0, 0, 0, 0.5), rgba(255, 0, 0, 0.5)), rgba(255, 255, 255, 0.1);
